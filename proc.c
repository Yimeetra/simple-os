...
void init_proc_stack(struct proc *proc, void (*entry)(), uint32_t *stack_top) {
    proc->context.edi    = 0;
    proc->context.esi    = 0;
    proc->context.ebp    = (uint32_t)stack_top;
    proc->context.esp    = (uint32_t)stack_top-3;
    proc->context.ebx    = 0;
    proc->context.edx    = 0;
    proc->context.ecx    = 0;
    proc->context.eax    = 0;
...
int32_t create_proc(void (*entry)()) {
    for (int i = 0; i < MAX_PROCS; ++i) {
        if (procs[i].state == PROC_FINISHED) {
            ...
            init_proc_stack(proc, entry, (uint32_t*) PROCS_SPACE_START + i*(PROC_STACK_SIZE + PROC_PADDING));
            proc->pid = i;
...
struct proc *sched() {
    ...
    for (int i = start; i < MAX_PROCS; ++i) {
        if (procs[i].wait_until > channel0.counter) {
            continue;
        }
        if (procs[i].state != PROC_FINISHED) {
...
void wait(float seconds) {
    current_proc->wait_until = seconds * channel0.frequency + channel0.counter;
    sleep(seconds);
}